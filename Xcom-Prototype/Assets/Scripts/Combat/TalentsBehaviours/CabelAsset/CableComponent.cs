...
public class CableComponent : MonoBehaviour
{
    ...
    public Transform[] BurningParticles;
	...
	void InitCableParticles()
	{
        ...
        BurningParticles = new Transform[segments + 1];
        Transform deformationSystem = transform.parent.Find("DeformationSystem");
        if (deformationSystem != null)
        {
            Transform rootM = deformationSystem.Find("Root_M");
            if (rootM != null)
            {
                for (int i = 0; i <= segments; i++)
                {
                    string jointName = $"whipjoint{i:D2}_M";
                    Transform jointTransform = rootM.Find(jointName);
                    if (jointTransform != null)
                    {
                        BurningParticles[i] = jointTransform;
                    }
                }
            }
        }
	...
	void RenderCable()
	{
        for (int pointIdx = 0; pointIdx < segments + 1; pointIdx++)
        {
            ...
            if (BurningParticles[pointIdx] != null)
            {
                BurningParticles[pointIdx].position = points[pointIdx].Position;
            }
        }
    }
