...
public class EarthBallTalentBehaviour : BaseSkillBehaviour
{
    private BaseProjectile.Factory projectileFactory;

    private BaseCharacerView _selfCharacter;
    private Collider[] colliders;

    public EarthBallTalentBehaviour(BaseSkillModel skill, BaseParticleView.Factory particleFactory, CharactersRegistry charactersRegistry, BaseProjectile.Factory projectileFactory, BaseDecale.Factory decaleFactory) : base(skill, particleFactory, charactersRegistry)
    {
        ...
        if (colliders.Length != 0)
        {
            foreach (Collider collider in colliders)
            {
                ...
                if (damageble != null && damageble is BaseCharacerView target && !_selfCharacter.IsMyTeammate(target))
                {
                    ...
                    var armorDebuff = new ArmorDebuff(_particleFactory, Skill);
                    armorDebuff.damage = damage;
                    target.SkillServiceProvider.AddBuff(armorDebuff);
                }
            }
        }
    }
