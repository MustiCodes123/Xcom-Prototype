...
public class NavmeshPathDraw : MonoBehaviour
{
    ...
    public void Draw()
    {
        ...
        if (Physics.Raycast(destinationPos, -Vector3.up, out downHit, Mathf.Infinity, groundLayers)) {
            validatedDesPos = new Vector3(destinationPos.x, downHit.transform.position.y, destinationPos.z);
        }else{
            validatedDesPos = destinationPos;
        }
